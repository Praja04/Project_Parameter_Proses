...
{
    protected $table            = 'line6_data_hsm2';

    protected $allowedFields = [
        'L6_HSM2_TEMP_LEFT',
        'L6_HSM2_TEMP_RIGHT',
        'L6_HSM2_TEMP_SET_VALUE',
        'L6_HSM2_LID_HOLDER_ACTUAL_POS',
        'L6_HSM2_LID_HOLDER_WAITING_POS',
        'L6_HSM2_LID_HOLDER_HOLDING_POS',
        'L6_HSM2_LID_HOLDER_MELTING_POS',
        'L6_HSM2_LID_HOLDER_SEALING_POS',
        'L6_HSM2_BOX_LIFTER_ACTUAL_POS',
        'L6_HSM2_BOX_LIFTER_WAITING_POS',
        'L6_HSM2_BOX_LIFTER_MELTING_POS',
        'L6_HSM2_BOX_LIFTER_SEALING_POS',
        'L6_HSM2_BOX_LIFTER_FEEDING_POS',
        'L6_HSM2_MIRROR_ACTUAL_POS',
        'L6_HSM2_MIRROR_WAITING_POS',
        'L6_HSM2_MIRROR_MELTING_POS',
        'UPPER_LIMIT_TEMP',
        'LOWER_LIMIT_TEMP',
        'L6_HSM2_TYPE_BATTERY',
        'L6_HSM2_LID_MELTING_TIME',
        'L6_HSM2_BOX_MELTING_TIME',
        'L6_HSM2_SEALING_TIME',
        'waktu'
    ];

    public function getDataTempLeft_HSM2()
    {
        return $this->select('L6_HSM2_TYPE_BATTERY,L6_HSM2_TEMP_LEFT, L6_HSM2_TEMP_SET_VALUE,waktu')
            ->orderBy('waktu', 'DESC')
            ->limit(50)
            ->find();
    }
    ...
    {
        return $this->select('L6_HSM2_TYPE_BATTERY,L6_HSM2_TEMP_RIGHT, L6_HSM2_TEMP_SET_VALUE,waktu')
            ->orderBy('waktu', 'DESC')
            ->limit(50)
            ->find();
    }
    ...
    public function getDataTempLeftByDate($date)
    {
        ...
        $filteredData = [];
        foreach ($data as $row) {
            $batteryType = $row['L6_HSM2_TYPE_BATTERY'];
            $tempRight = $row['L6_HSM2_TEMP_LEFT'];
    ...
    public function getDataTempRightByDate($date)
    {
        ...
        // ->orderBy('waktu', 'DESC')
        // ->where('DATE(waktu)', $date)->findAll();
        // Ensure $date is in the format 'Y-m-d'
        $startTime = $date . ' 07:30:00.000';
        $endTime = date('Y-m-d H:i:s.u', strtotime($date . ' +1 day 07:29:00'));
        ...
        foreach ($data as $row) {
            $batteryType = $row['L6_HSM2_TYPE_BATTERY'];
            $tempRight = $row['L6_HSM2_TEMP_RIGHT'];
    ...
    public function getBatteryTypeByDate($batteryType)
    {
        ...
        return $this->where('L6_HSM2_TYPE_BATTERY', $batteryType)
            ->where('DATE(waktu)', $today)
            ->orderBy('waktu', 'ASC')
            ->findAll();
    }
    ...
    public function getBatteryTypeByDateFirst($batteryType)
    {
        ...
        return $this->select('L6_HSM2_TYPE_BATTERY, waktu')
            ->where('L6_HSM2_TYPE_BATTERY', $batteryType)
            ->where('DATE(waktu)', $today)
            ->orderBy('waktu', 'ASC')
            ->first();
    }
    ...
    public function getLastBatteryTypeTime()
    {
        ...
        //             L6_HSM2_TYPE_BATTERY, 
        //             waktu, 
        //             ROW_NUMBER() OVER (PARTITION BY L6_HSM2_TYPE_BATTERY ORDER BY waktu DESC) AS rn 
        //         FROM (
        //             SELECT 
        //                 L6_HSM2_TYPE_BATTERY, 
        //                 waktu, 
        //                 LAG(L6_HSM2_TYPE_BATTERY) OVER (ORDER BY waktu) AS previous_battery_type 
        //             FROM 
        //                 line6_data_hsm2
        //         ) AS changes 
        //         WHERE 
        //             L6_HSM2_TYPE_BATTERY <> previous_battery_type 
    ...
    SELECT 
        CASE 
            WHEN L6_HSM2_TYPE_BATTERY IN ('0', '2') THEN '0'  -- Treat '0' and '2' as the same
            ELSE L6_HSM2_TYPE_BATTERY 
        END AS normalized_battery_type,
        waktu, 
        ROW_NUMBER() OVER (PARTITION BY CASE 
                                            WHEN L6_HSM2_TYPE_BATTERY IN ('0', '2') THEN '0' 
                                            ELSE L6_HSM2_TYPE_BATTERY 
                                         END 
    ...
    FROM (
        SELECT 
            ...
            waktu, 
            LAG(L6_HSM2_TYPE_BATTERY) OVER (ORDER BY waktu) AS previous_battery_type 
        FROM line6_data_hsm2 
    ) AS changes 
    WHERE L6_HSM2_TYPE_BATTERY <> previous_battery_type OR previous_battery_type IS NULL
) 
SELECT 
    waktu, 
    normalized_battery_type AS L6_HSM2_TYPE_BATTERY 
FROM BatteryChanges 
WHERE rn = 1;
        ");
    ...
    public function getDataGreen()
    {
        ...
        foreach ($shifts as $key => $shift) {
            $count = $this->where('DATE(waktu)', $today)
                ->where('L6_HSM2_TEMP_LEFT <', '435')
                ->where('L6_HSM2_TEMP_LEFT >', '415')
                ->where('L6_HSM2_TEMP_RIGHT <', '435')
                ->where('L6_HSM2_TEMP_RIGHT >', '415')
                ->where('TIME(waktu) >=', $shift['start'])
                ->where('TIME(waktu) <=', $shift['end'])
                ->countAllResults();
    ...
    public function getDataYellow()
    {
        ...
        foreach ($shifts as $key => $shift) {
            $count = $this->where('DATE(waktu)', $today)
                ->where('L6_HSM2_TEMP_LEFT <', '440')
                ->where('L6_HSM2_TEMP_RIGHT <', '440')
                ->where('TIME(waktu) >=', $shift['start'])
                ->where('TIME(waktu) <=', $shift['end'])
                ->countAllResults();
