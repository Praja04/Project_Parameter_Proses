...
{
    protected $table            = 'line5_data_hsm1';

    protected $allowedFields = [
        'L5_HSM1_TEMP_LEFT',
        'L5_HSM1_TEMP_RIGHT',
        'L5_HSM1_TEMP_SET_VALUE',
        'L5_HSM1_LID_HOLDER_ACTUAL_POS',
        'L5_HSM1_LID_HOLDER_WAITING_POS',
        'L5_HSM1_LID_HOLDER_HOLDING_POS',
        'L5_HSM1_LID_HOLDER_MELTING_POS',
        'L5_HSM1_LID_HOLDER_SEALING_POS',
        'L5_HSM1_BOX_LIFTER_ACTUAL_POS',
        'L5_HSM1_BOX_LIFTER_WAITING_POS',
        'L5_HSM1_BOX_LIFTER_MELTING_POS',
        'L5_HSM1_BOX_LIFTER_SEALING_POS',
        'L5_HSM1_BOX_LIFTER_FEEDING_POS',
        'L5_HSM1_MIRROR_ACTUAL_POS',
        'L5_HSM1_MIRROR_WAITING_POS',
        'L5_HSM1_MIRROR_MELTING_POS',
        'UPPER_LIMIT_TEMP',
        'LOWER_LIMIT_TEMP',
        'L5_HSM1_TYPE_BATTERY',
        'L5_HSM1_LID_MELTING_TIME',
        'L5_HSM1_BOX_MELTING_TIME',
        'L5_HSM1_SEALING_TIME',
        'waktu'
    ];

    public function getDataTempLeft_HSM1()
    {
        return $this->select('L5_HSM1_TEMP_LEFT, L5_HSM1_TEMP_SET_VALUE,waktu')
            ->orderBy('waktu', 'DESC')
            ->limit(50)
            ->find();
    }
    ...
    {
        return $this->select('L5_HSM1_TEMP_RIGHT, L5_HSM1_TEMP_SET_VALUE,waktu')
            ->orderBy('waktu', 'DESC')
            ->limit(50)
            ->find();
    }
    ...
    public function getDataTempLeftByDate($date)
    {
        ...
        $filteredData = [];
        foreach ($data as $row) {
            $batteryType = $row['L5_HSM1_TYPE_BATTERY'];
            $tempRight = $row['L5_HSM1_TEMP_LEFT'];
    ...
    public function getDataTempRightByDate($date)
    {
        ...
        foreach ($data as $row) {
            $batteryType = $row['L5_HSM1_TYPE_BATTERY'];
            $tempRight = $row['L5_HSM1_TEMP_RIGHT'];
    ...
    public function getBatteryTypeByDate($batteryType)
    {
        ...
        return $this->where('L5_HSM1_TYPE_BATTERY', $batteryType)
    ...
    public function getBatteryTypeByDateFirst($batteryType)
    {
        ...
        return $this->select('L5_HSM1_TYPE_BATTERY, waktu')
            ->where('L5_HSM1_TYPE_BATTERY', $batteryType)
            ->where('DATE(waktu)', $today)
            ->orderBy('waktu', 'ASC')
            ->first();
    }
        ...
        $query = $this->db->query("
            WITH BatteryChanges AS (
                SELECT 
                    L5_HSM1_TYPE_BATTERY, 
                    waktu, 
                    ROW_NUMBER() OVER (PARTITION BY L5_HSM1_TYPE_BATTERY ORDER BY waktu DESC) AS rn 
                FROM (
                    SELECT 
                        L5_HSM1_TYPE_BATTERY, 
                        waktu, 
                        LAG(L5_HSM1_TYPE_BATTERY) OVER (ORDER BY waktu) AS previous_battery_type 
                    FROM 
                        line5_data_hsm1
                ) AS changes 
                WHERE 
                    L5_HSM1_TYPE_BATTERY <> previous_battery_type 
    ...
    public function getDataGreen()
    {
        ...
        foreach ($shifts as $key => $shift) {
            $count = $this->where('DATE(waktu)', $today)
                ->where('L5_HSM1_TEMP_LEFT <', '410')
                ->where('L5_HSM1_TEMP_LEFT >', '395')
                ->where('L5_HSM1_TEMP_RIGHT <', '410')
                ->where('L5_HSM1_TEMP_RIGHT >', '395')
                ->where('TIME(waktu) >=', $shift['start'])
                ->where('TIME(waktu) <=', $shift['end'])
                ->countAllResults();
    ...
    public function getDataYellow()
    {
        ...
        foreach ($shifts as $key => $shift) {
            $count = $this->where('DATE(waktu)', $today)
                ->where('L5_HSM1_TEMP_LEFT <', '440')
                ->where('L5_HSM1_TEMP_RIGHT <', '440')
                ->where('TIME(waktu) >=', $shift['start'])
                ->where('TIME(waktu) <=', $shift['end'])
                ->countAllResults();
    ...
    public function getDistinctTempLeftbyDate($date)
    {
        ...
        return $this->select("CASE 
                                ...
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 370 AND 379.9 THEN '370-379.9' 
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 380 AND 389.9 THEN '380-389.9' 
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 390 AND 399.9 THEN '390-399.9' 
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 400 AND 409.9 THEN '400-409.9' 
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 410 AND 419.9 THEN '410-419.9' 
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 420 AND 429.9 THEN '420-429.9' 
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 430 AND 439.9 THEN '430-439.9' 
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 440 AND 449.9 THEN '440-449.9' 
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 450 AND 459.9 THEN '450-459.9' 
                                WHEN L5_HSM1_TEMP_LEFT BETWEEN 460 AND 470.9 THEN '460-470.9' 
                                ELSE 'OTHER' 
    ...
    public function getDistinctTempRightbyDate($date)
    {
        ...
        return $this->select("CASE 
                                ...
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 370 AND 379.9 THEN '370-379.9' 
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 380 AND 389.9 THEN '380-389.9' 
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 390 AND 399.9 THEN '390-399.9' 
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 400 AND 409.9 THEN '400-409.9' 
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 410 AND 419.9 THEN '410-419.9' 
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 420 AND 429.9 THEN '420-429.9' 
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 430 AND 439.9 THEN '430-439.9' 
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 440 AND 449.9 THEN '440-449.9' 
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 450 AND 459.9 THEN '450-459.9' 
                                WHEN L5_HSM1_TEMP_RIGHT BETWEEN 460 AND 470.9 THEN '460-470.9' 
                                ELSE 'OTHER' 
