    ...
    const fetchDataLeft1 = () => {
        $.ajax({
            ...
            success: (data) => {
                ...
                const valuesSet = data.data.map(item => parseFloat(item.L6_HSM1_TEMP_SET_VALUE)); // Konversi ke angka
                const valuesLeft = data.data.map(item => parseFloat(item.L6_HSM1_TEMP_LEFT)); // Konversi ke angka
                ...
                if (chart3) {
                    ...
                    chart3 = Highcharts.chart('container', {
                        ...
                        title: {
                            text: 'L6_HSM1_TEMP_SET_VALUE dan L6_HSM1_TEMP_LEFT'
                        },
                        xAxis: {
                            labels: {
                                enabled: false,
                        ...
                        series: [{
                            name: 'L6_HSM1_TEMP_SET_VALUE',
                            data: valuesSet
                        }, {
                            name: 'L6_HSM1_TEMP_LEFT',
                            data: valuesLeft
                        }]
                    });
                }
...
<!-- <script>
    ...
    const fetchDataCell4 = () => {
        $.ajax({
            ...
            success: (data) => {
                const labels = data.data.map(item => item.waktu); // Mengakses waktu
                const valuesRight = data.data.map(item => parseFloat(item.L6_HSM1_TEMP_RIGHT)); // Konversi ke angka
                const valuesSet = data.data.map(item => parseFloat(item.L6_HSM1_TEMP_SET_VALUE)); // Konversi ke angka
                ...
                if (chart4) {
                    ...
                    chart4 = Highcharts.chart('container2', {
                        ...
                        title: {
                            text: 'L6_HSM1_TEMP_RIGHT dan L6_HSM1_TEMP_SET_VALUE'
                        },
                        xAxis: {
                            labels: {
                                enabled: false,
    ...
    const fetchDataRight1 = () => {
        $.ajax({
            ...
            success: (data) => {
                ...
                updateOrCreateChart('container', chart, 'TEMP RIGHT', labels, valuesSet, 'L6_HSM1_TEMP_SET_VALUE', valuesRight, 'L6_HSM1_TEMP_RIGHT', (newChart) => {
                    chart = newChart;
                });
            },
            error: (jqXHR, textStatus, errorThrown) => {
