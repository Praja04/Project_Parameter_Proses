            ...
            <div class="col-xl-12 col-12" style="background-color:#03346E;">
                <div class="row d-flex flex-row" style="margin: 10px;">
                    ...
                    <div class="col-md-3-5">
                        <div class="box-body" style="background-color:#F2EFE5;">
                            <h5 class="text-center">APB 2 Line 5 Detail</h5>
...
<script>
    ...
    const fetchDataByWeek = (selectedDate, selectedDate2) => {
        ...
        $.ajax({
            url: `<?= base_url('apbline5/data/temp/left2/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
            success: function(data) {
                ...
                updateOrCreateChartweek(valuesStandardMinimal, valuesStandardMaksimal, 'container2', chart3, 'APB2 - TEMP LEFT Actual', labels, valuesSet, 'L5_APB2_TEMP_LEFT_SETTING', valuesLeft, 'L5_APB2_TEMP_LEFT_ACTUAL', (newChart) => {
                    chart3 = newChart;
                }, filteredLabels, data.data.map(item => item.waktu.split(' ')[1]));
            },
            error: (jqXHR, textStatus, errorThrown) => {
        ...
        $.ajax({
            url: `<?= base_url('apbline5/data/temp/right2/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
            success: function(data) {
                ...
                updateOrCreateChartweek(valuesStandardMinimal, valuesStandardMaksimal, 'container', chart, 'APB2 - TEMP RIGHT Actual', labels, valuesSet, 'L5_APB2_TEMP_RIGHT_SETTING', valuesRight, 'L5_APB2_TEMP_RIGHT_ACTUAL', (newChart) => {
                    chart = newChart;
                }, filteredLabels, data.data.map(item => item.waktu.split(' ')[1]));
            },
            error: (jqXHR, textStatus, errorThrown) => {
...
<script>
    ...
    const fetchData_tempLeft = () => {
        $.ajax({
            url: '<?= base_url('apbline5/distinct/left2') ?>',
            method: 'GET',
            dataType: 'json',
            success: (data) => {
    ...
    const fetchData_tempRight = () => {
        $.ajax({
            url: '<?= base_url('apbline5/distinct/right2') ?>',
            method: 'GET',
            dataType: 'json',
            success: (data) => {
    ...
    const fetchData_tempByDate = (date) => {
        ...
        $.ajax({
            url: `<?= base_url('apbline5/distinct/right2/date/') ?>${date}`,
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                createChart('temp_right', `<?= base_url('apbline5/distinct/right2/date/') ?>${date}`, 'TEMP_Actual_RIGHT_RANGE', 'Temperatur Right');
            },
            error: (jqXHR, textStatus, errorThrown) => {
                console.error('Error fetching data for temp right apb 2:', textStatus, errorThrown);
            }
        ...
        $.ajax({
            url: `<?= base_url('apbline5/distinct/left2/date/') ?>${date}`,
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                createChart('temp_left', `<?= base_url('apbline5/distinct/left2/date/') ?>${date}`, 'TEMP_Actual_LEFT_RANGE', 'Temperatur Left');
            },
            error: (jqXHR, textStatus, errorThrown) => {
                console.error('Error fetching data for temp left apb 2:', textStatus, errorThrown);
            }
...
<script>
    ...
    function displayData(response) {
        ...
        $('#standar_first_dipping_time').val(response.data['L5_APB2_FIRST_DIPPING_TIME']);
        $('#standar_weld_head_down_time').val(response.data['L5_APB2_WELD_HEAD_DOWN_TIME']);
        $('#standar_flame_waiting_time').val(response.data['L5_APB2_FLAME_WAITING_TIME']);

        $('#actual_temp_right').val(response.data['L5_APB2_TEMP_RIGHT_ACTUAL']);
        $('#actual_temp_left').val(response.data['L5_APB2_TEMP_LEFT_ACTUAL']);
        $('#actual_cooling_time').val(response.data['L5_APB2_COOLING_TIME']);
        $('#actual_first_dipping_time').val(response.data['L5_APB2_FIRST_DIPPING_TIME']);
        $('#actual_weld_head_down_time').val(response.data['L5_APB2_WELD_HEAD_DOWN_TIME']);
        $('#actual_flame_waiting_time').val(response.data['L5_APB2_FLAME_WAITING_TIME']);

    }

    function fetchData() {
        $.ajax({
            url: '<?= base_url('apbline5/get/parameter/2') ?>',
            method: 'GET',
