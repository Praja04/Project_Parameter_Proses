...
<script>
    ...
    const fetchDataLidHolder = () => {
        $.ajax({
            url: '<?= base_url('hsmline5/get/data/lid_holder_melting/1') ?>',
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                ...
                const valuesActual = data.data.map(item => parseFloat(item.L5_HSM1_LID_HOLDER_ACTUAL_POS));
                const valuesMelting = data.data.map(item => parseFloat(item.L5_HSM1_LID_HOLDER_MELTING_POS));


                // Create upper and lower arrays based on valuesStandar

                updateOrCreateChartMelting('lidholder', chart_lidholder, 'Lid Holder Position', labels, valuesActual, 'L5_HSM1_LID_HOLDER_ACTUAL_POS', valuesMelting, 'L5_HSM1_LID_HOLDER_MELTING_POS', (newChart) => {
    ...
    const fetchDataBoxLifter = () => {
        $.ajax({
            url: '<?= base_url('hsmline5/get/data/box_lifter_melting/1') ?>',
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                const time = data.data.map(item => item.waktu);
                ...
                const labels = data.data.map(item => item.waktu);
                const valuesActual = data.data.map(item => parseFloat(item.L5_HSM1_BOX_LIFTER_ACTUAL_POS));
                const valuesMelting = data.data.map(item => parseFloat(item.L5_HSM1_BOX_LIFTER_MELTING_POS));


                // Create upper and lower arrays based on valuesStandar

                updateOrCreateChartMelting('boxlifter', chart_boxlifter, 'Box Lifter Position', labels, valuesActual, 'L5_HSM1_BOX_LIFTER_ACTUAL_POS', valuesMelting, 'L5_HSM1_BOX_LIFTER_MELTING_POS', (newChart) => {
    ...
    const fetchDataMirrorPos = () => {
        $.ajax({
            url: '<?= base_url('hsmline5/get/data/mirror_pos/1') ?>',
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                const time = data.data.map(item => item.waktu);
                ...
                const labels = data.data.map(item => item.waktu);
                const valuesActual = data.data.map(item => parseFloat(item.L5_HSM1_MIRROR_ACTUAL_POS));
                const valuesMelting = data.data.map(item => parseFloat(item.L5_HSM1_MIRROR_MELTING_POS));


                // Create upper and lower arrays based on valuesStandar

                updateOrCreateChartMelting('mirrorpos', chart_mirror, 'Mirror Position', labels, valuesActual, 'L5_HSM1_MIRROR_ACTUAL_POS', valuesMelting, 'L5_HSM1_MIRROR_MELTING_POS', (newChart) => {
    ...
    const fetchDataByDate = (date) => {
        // Fetch untuk cell1
        $.ajax({
            url: `<?= base_url('hsmline5/get/temp/right1/date/') ?>${date}`,
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                const time = data.data.map(item => item.waktu);
                ...
                console.log(filteredLabels);
                const labels = data.data.map(item => item.waktu);
                const valuesSet = data.data.map(item => parseFloat(item.L5_HSM1_TEMP_SET_VALUE));
                const valuesRight = data.data.map(item => parseFloat(item.L5_HSM1_TEMP_RIGHT));
                const valuesStandar = data.data.map(item => parseFloat(item.L5_HSM1_TYPE_BATTERY));

                // Create upper and lower arrays based on valuesStandar
                const upper = valuesStandar.map(value => (value === 0 || value === 2) ? 435 : 425);
                const lower = valuesStandar.map(value => (value === 0 || value === 2) ? 415 : 405);
                updateOrCreateChart('container', chart, 'TEMP RIGHT', labels, valuesSet, 'L5_HSM1_TEMP_SET_VALUE', valuesRight, 'L5_HSM1_TEMP_RIGHT', (newChart) => {
                    chart = newChart;
        ...
        // Fetch untuk cell3
        $.ajax({
            url: `<?= base_url('hsmline5/get/temp/left1/date/') ?>${date}`,
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                const time = data.data.map(item => item.waktu);
                ...
                const labels = data.data.map(item => item.waktu);
                const valuesSet = data.data.map(item => parseFloat(item.L5_HSM1_TEMP_SET_VALUE));
                const valuesLeft = data.data.map(item => parseFloat(item.L5_HSM1_TEMP_LEFT));
                const valuesStandar = data.data.map(item => parseFloat(item.L5_HSM1_TYPE_BATTERY));

                // Create upper and lower arrays based on valuesStandar
                const upper = valuesStandar.map(value => (value === 0 || value === 2) ? 435 : 425);
                const lower = valuesStandar.map(value => (value === 0 || value === 2) ? 415 : 405);
                updateOrCreateChart('container2', chart3, 'TEMP LEFT', labels, valuesSet, 'L5_HSM1_TEMP_SET_VALUE', valuesLeft, 'L5_HSM1_TEMP_LEFT', (newChart) => {
                    chart3 = newChart;
                }, upper, lower, filteredLabels, data.data.map(item => item.waktu.split(' ')[1]));
            },
            error: (jqXHR, textStatus, errorThrown) => {
        ...
        //lidholder
        $.ajax({
            url: `<?= base_url('hsmline5/get/data/lid_holder_melting1/date/') ?>${date}`,
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                const time = data.data.map(item => item.waktu);
        ...
        //boxlifter
        $.ajax({
            url: `<?= base_url('hsmline5/get/data/box_lifter_melting1/date/') ?>${date}`,
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                const time = data.data.map(item => item.waktu);
        ...
        //mirror pos
        $.ajax({
            url: `<?= base_url('hsmline5/get/data/mirror_pos1/date/') ?>${date}`,
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                const time = data.data.map(item => item.waktu);
    ...
    const fetchDataRight1 = () => {
        $.ajax({
            url: '<?= base_url('hsmline5/get/temp/right1') ?>',
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                const time = data.data.map(item => item.waktu);
    ...
    const fetchDataLeft1 = () => {
        $.ajax({
            url: '<?= base_url('hsmline5/get/temp/left1') ?>',
            method: 'GET',
            dataType: 'json',
            success: (data) => {
                const time = data.data.map(item => item.waktu);
    ...
    const fetchDataByWeek = (selectedDate, selectedDate2) => {
        ...
        $.ajax({
            url: `<?= base_url('hsmline5/data/temp/right1/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
            success: function(data) {
                ...
                const upper = valuesStandar.map(value => (value === 0 || value === 2) ? 435 : 425);
                const lower = valuesStandar.map(value => (value === 0 || value === 2) ? 415 : 405);
                updateOrCreateChartweek('container', chart, 'TEMP RIGHT', labels, valuesSet, 'L5_HSM1_TEMP_SET_VALUE', valuesRight, 'L5_HSM1_TEMP_RIGHT', (newChart) => {
                    chart = newChart;
                }, upper, lower, filteredLabels, data.data.map(item => item.waktu.split(' ')[1]));
            },
            error: (jqXHR, textStatus, errorThrown) => {
        ...
        $.ajax({
            url: `<?= base_url('hsmline5/data/temp/left1/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
            success: function(data) {
                ...
                const upper = valuesStandar.map(value => (value === 0 || value === 2) ? 435 : 425);
                const lower = valuesStandar.map(value => (value === 0 || value === 2) ? 415 : 405);
                updateOrCreateChartweek('container2', chart, 'TEMP LEFT', labels, valuesSet, 'L5_HSM1_TEMP_SET_VALUE', valuesLeft, 'L5_HSM1_TEMP_LEFT', (newChart) => {
                    chart = newChart;
                }, upper, lower, filteredLabels, data.data.map(item => item.waktu.split(' ')[1]));
            },
            error: (jqXHR, textStatus, errorThrown) => {
        ...
        $.ajax({
            url: `<?= base_url('hsmline5/data/lid_holder1/pos/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
            success: function(data) {
                ...
                updateOrCreateChartMeltingweek('lidholder', chart_lidholder, 'Lid Holder Position', labels, valuesActual, 'L5_HSM1_LID_HOLDER_ACTUAL_POS', valuesMelting, 'L5_HSM1_LID_HOLDER_MELTING_POS', (newChart) => {
                    chart_lidholder = newChart;
                }, filteredLabels, data.data.map(item => item.waktu.split(' ')[1]));
        ...
        $.ajax({
            url: `<?= base_url('hsmline5/data/box_lifter1/pos/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
            success: function(data) {
                ...
                updateOrCreateChartMeltingweek('boxlifter', chart_boxlifter, 'Box Lifter Position', labels, valuesActual, 'L5_HSM1_BOX_LIFTER_ACTUAL_POS', valuesMelting, 'L5_HSM1_BOX_LIFTER_MELTING_POS', (newChart) => {
                        chart_boxlifter = newChart;
                    },
                    filteredLabels, data.data.map(item => item.waktu.split(' ')[1]));
        ...
        $.ajax({
            url: `<?= base_url('hsmline5/data/mirror1/pos/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
            success: function(data) {
                ...
                updateOrCreateChartMeltingweek('mirrorpos', chart_mirror, 'Mirror Position', labels, valuesActual, 'L5_HSM1_MIRROR_ACTUAL_POS', valuesMelting, 'L5_HSM1_MIRROR_MELTING_POS', (newChart) => {
                    chart_mirror = newChart;
                }, filteredLabels, data.data.map(item => item.waktu.split(' ')[1]));
...
<script>
    ...
    $.ajax({
        url: '<?= base_url('hsmline5/get/type/1') ?>',
        method: 'GET',
        success: function(response) {
            // Filter data untuk tipe 1 dan 2
            const filteredData = response.data.filter(item =>
                item.L5_HSM1_TYPE_BATTERY === "1" || item.L5_HSM1_TYPE_BATTERY === "2"
            );
    ...
    $.ajax({
        url: '<?= base_url('hsmline5/get/data/ok/1') ?>',
        method: 'GET',
        success: function(response) {
...
<script>
    ...
    function displayData(response) {
        ...
        if (response.data['L5_HSM1_TYPE_BATTERY'] == '2') {
            $('#set_val_left').val('415-435');
            $('#set_val_right').val('415-435');
        } else if (response.data['L5_HSM1_TYPE_BATTERY'] == '1') {
            $('#set_val_left').val('395-410');
            $('#set_val_right').val('395-410');
        }

        $('#temp_left1').val(response.data['L5_HSM1_TEMP_LEFT']);
        $('#temp_right1').val(response.data['L5_HSM1_TEMP_RIGHT']);
        $('#set_value1').val(response.data['L5_HSM1_TEMP_SET_VALUE']);
        $('#set_value2').val(response.data['L5_HSM1_TEMP_SET_VALUE']);
        $('#standar_lid_holder_melting_pos').val(response.data['L5_HSM1_LID_HOLDER_MELTING_POS']);
        $('#actual_lid_holder_melting_pos').val(response.data['L5_HSM1_LID_HOLDER_MELTING_POS']);
        $('#actual_lid_holder_sealing_pos').val(response.data['L5_HSM1_LID_HOLDER_SEALING_POS']);
        $('#standar_lid_holder_sealing_pos').val(response.data['L5_HSM1_LID_HOLDER_SEALING_POS']);
        $('#standar_box_lifter_melting_pos').val(response.data['L5_HSM1_BOX_LIFTER_MELTING_POS']);
        $('#actual_box_lifter_melting_pos').val(response.data['L5_HSM1_BOX_LIFTER_MELTING_POS']);
        $('#standar_box_lifter_sealing_pos').val(response.data['L5_HSM1_BOX_LIFTER_SEALING_POS']);
        $('#actual_box_lifter_sealing_pos').val(response.data['L5_HSM1_BOX_LIFTER_SEALING_POS']);
        $('#standar_mirror_melting_pos').val(response.data['L5_HSM1_MIRROR_MELTING_POS']);
        $('#actual_mirror_melting_pos').val(response.data['L5_HSM1_MIRROR_MELTING_POS']);
        $('#actual_lid_melting_time').val(response.data['L5_HSM1_LID_MELTING_TIME']);
        $('#actual_box_melting_time').val(response.data['L5_HSM1_BOX_MELTING_TIME']);
        $('#actual_sealing_time').val(response.data['L5_HSM1_SEALING_TIME']);
    ...
    function fetchData() {
        $.ajax({
            url: '<?= base_url('hsmline5/get/data/parameter/1') ?>',
            method: 'GET',
            success: function(response) {
