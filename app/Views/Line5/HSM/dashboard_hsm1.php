    ...
    const fetchDataRight1 = () => {
        $.ajax({
            ...
            success: (data) => {
                ...
                const valuesSet = data.data.map(item => parseFloat(item.L5_HSM1_TEMP_SET_VALUE) || 0);
                const valuesRight = data.data.map(item => parseFloat(item.L5_HSM1_TEMP_RIGHT) || 0);
    ...
    const fetchDataLeft1 = () => {
        $.ajax({
            ...
            success: (data) => {
                const labels = data.data.map(item => item.waktu);
                const valuesSet = data.data.map(item => parseFloat(item.L5_HSM1_TEMP_SET_VALUE) || 0);
                const valuesLeft = data.data.map(item => parseFloat(item.L5_HSM1_TEMP_LEFT) || 0);
